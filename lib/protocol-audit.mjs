...
export async function audit({ offline = false } = {}) {
  ...
  // COVERAGE CHECKS RETIRED 2026-09-11 along with coverage.mjs. They enforced a
  // full watchlist sweep before deploying — a protocol that ran ~1,200 searches in
  // 22 days and produced -1.94%. The situation framework that replaced it does not
  // sweep on a schedule; it hunts for named mechanisms, and gating that on a stale
  // timestamp only blocked the trades this agent exists to make.
  ...
  // ---- 5b. Candidates must be instruments that could ACTUALLY be traded. --
  //
  // Runs 10-12 listed perp tickers in `tickers_considered` and rejected the
  // headline ideas (a SOXSX semiconductor short, a BRENTOIL oil short) on
  // "liquidation risk" — a judgment rationale. The real reason was harder: the
  // futures account is EMPTY (`/uapi/v1/account/balances` -> balances:[]), there
  // is no transfer command anywhere in the CLI, and TRANSFER is outside the
  // mandate. Those orders could never have been placed at all.
  //
  // Dressing a hard blocker as a risk judgment is exactly the self-reported
  // failure this file exists to catch, and nothing else could catch it, because
  // every other gate only ever sees orders that DID happen.
  if (!offline && cov?.themes) {
    let perpFunded = null;
    try {
      const raw = execFileSync("node lib/futures.mjs GET /uapi/v1/account/balances",
        { encoding: "utf8", cwd: ROOT, stdio: ["ignore", "pipe", "ignore"], shell: true });
      const j = JSON.parse(raw);
      const b = j?.data?.balances ?? [], iso = j?.data?.isolates ?? [];
      perpFunded = b.length > 0 || iso.length > 0;
    } catch { /* leave null — unknown, do not assert either way */ }

    if (perpFunded === false) {
      const perps = new Set();
      for (const t of Object.values(cov.themes)) {
        for (const sym of t?.tickers_considered ?? []) if (/_PERP$/.test(String(sym))) perps.add(sym);
      }
      const ack = String(cov.perp_account_unfunded_ack ?? "").trim();
      if (perps.size && ack.length < 60) {
        problems.push(
          `UNFUNDABLE CANDIDATES: ${perps.size} PERP ticker(s) are listed in tickers_considered ` +
          `(${[...perps].slice(0, 4).join(", ")}${perps.size > 4 ? ", …" : ""}) but the futures account is EMPTY and ` +
          `there is no transfer path — these orders CANNOT be placed. Listing them as considered candidates, or ` +
          `rejecting them on risk grounds, misrepresents a hard blocker as a judgment call. Either record ` +
          `"perp_account_unfunded_ack" in coverage.json stating plainly that the whole perp universe is ` +
          `unreachable this run, or hand Erik the manual steps to fund it.`);
      }
    }
  }
