...
/**
 * How many distinct tickers the watchlist names, across spot and perps.
 * Derived, never hardcoded, so growing the watchlist automatically raises the
 * scan floor rather than silently leaving new names uncovered.
 */
export function watchlistSymbolCount() {
  const w = JSON.parse(readFileSync(p("watchlist.json"), "utf8"));
  const seen = new Set();
  for (const theme of Object.values(w.tradeable ?? {})) {
    for (const key of ["spot", "perp_only"]) {
      for (const sym of theme?.[key] ?? []) seen.add(String(sym).replace(/_USDT(_PERP)?$/, ""));
    }
  }
  return seen.size;
}
...
export function checkCoverage(maxAgeMin = 90) {
  ...
  // ---- Minimum sweep depth. Erik, 2026-08-21. --------------------------
  // Runs 6-9 "covered" ~79 tickers with roughly one search per theme, so 30
  // semiconductor names shared a single digest and a name nobody was thinking
  // about could have moved 10% unseen. A price scan is cheap — these are API
  // calls, not searches — so there is no excuse for a partial one. Enforced
  // here rather than left in CLAUDE.md because prose there has been skipped
  // before and a gate has not.
  const need = watchlistSymbolCount();
  const scanned = Number(cov.symbols_scanned);
  if (!Number.isFinite(scanned)) {
    problems.push(`symbols_scanned missing — price-scan every watchlist symbol (${need} of them) and record the count.`);
  } else if (scanned < need) {
    problems.push(`symbols_scanned ${scanned} < ${need} watchlist symbols — scan all of them, not just the ones already on your mind.`);
  }

  // An unexplained mover is a hole in the news sweep, not a curiosity. The
  // field must be present and deliberate; an empty array is a valid answer,
  // a missing one is not.
  if (!Array.isArray(cov.unexplained_movers)) {
    problems.push("unexplained_movers missing — list every symbol that moved and could not be explained, or [] if none.");
  }
...
if (cmd === "init") {
  ...
  writeFileSync(COVERAGE, JSON.stringify({
    ...
    wide_sweep: "",
    symbols_scanned: 0,
    unexplained_movers: null,

    // ---- Fields enforced by lib/runcheck.mjs. See its header for why. -----
    // Scaffolded blank here so they are visible from the start of the run rather
    // than discovered at the end. runcheck only demands the first two once a
    // no-trade STREAK is running, and the third only if cash ends below one
    // venue minimum — but all three are cheap to fill in as you sweep.
    ranked_candidates: [],
    what_would_have_flipped_it: "",
    full_deployment_is_deliberate: "",
  }, null, 2));
  const need = watchlistSymbolCount();
  console.log(
    `scaffolded ${COVERAGE} with ${Object.keys(themes).length} themes — fill in every finding + verdict.\n` +
    `  also required: price-scan all ${need} watchlist symbols and set symbols_scanned, and set unexplained_movers ([] is fine).\n` +
    `  and at the END of the run: node lib/runcheck.mjs — audits the DECISION, not the sweep. It is the only\n` +
    `  check that looks at a do-nothing run, which preflight never sees.`,
  );
  process.exit(0);
}
