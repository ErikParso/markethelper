...
import { earnValueUsdt } from "./earn.mjs";
...
// ---- Earn: arbitrage stakes and dual-investment principal -------------------
// Locked capital is still capital. Leaving it out repeats the grid-bot bug.
const earn = await earnValueUsdt();
for (const e of earn.parts) positions.push({ kind: e.kind, coin: e.coin, qty: null, price: null, usdt: +e.usdt.toFixed(4) });

// Built from the parts. Perp notional is NOT added — only the margin and P&L are equity.
const equity = spotTotal + botTotal + futuresTotal + earn.total;
...
const out = {
  ...
  earn_usdt: +earn.total.toFixed(4),
...
if (process.argv.includes("--json")) {
  ...
  console.log(`EQUITY ${out.equity_usdt}  (spot cash ${out.cash_usdt} · bots ${out.bot_capital_usdt} · futures ${out.futures.total_usdt} · earn ${out.earn_usdt})`);
  ...
  for (const p of positions) {
    ...
    console.log(`  ${String(p.kind).padEnd(9)} ${String(p.coin).padEnd(16)} ${String(p.usdt).padStart(9)}  ${p.pct}%${extra}`);
  }
