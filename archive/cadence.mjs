...
if (input.next_event_utc) {
  ...
  if (!Number.isNaN(t)) {
    ...
    // Found at run 33: an event sitting just BEYOND the rule-based time is the bad case.
    // The rule said 6h, PCE landed at 7.5h, so the loop would have woken 1.5h BEFORE the
    // print — too early to know anything, and then needed a second run straight after.
    // If the event is within GRACE hours after the computed time, wait for the event
    // instead. Waking up just before a catalyst spends a run to learn nothing.
    const GRACE = 4 * HOUR;
    if (after.getTime() > Date.now() && after.getTime() < when.getTime()) {
      when = after;
      eventDriven = true;
      eventNote = `20 min after ${label} — the event, not the clock, sets this one`;
    } else if (after.getTime() > Date.now() && after.getTime() - when.getTime() < GRACE) {
      const waited = ((after.getTime() - when.getTime()) / HOUR).toFixed(1);
      when = after;
      eventDriven = true;
      eventNote = `waited an extra ${waited}h for ${label} rather than firing just before it — a run that wakes up shortly BEFORE a catalyst learns nothing and burns a slot`;
